...
$VMs = Get-AzResource -ResourceType "Microsoft.Compute/VirtualMachines" -TagName "VMSchedule-Enabled" -TagValue "Yes"
...
foreach ($VM in $VMs) {
    ...
    #Get the Value of the "VMSchedule-UTCOffset" Tag, that represents the offset from UTC
    $UTCOffset = $($VM.Tags)."VMSchedule-UTCOffset"
    ...
    $Exclude = $false
    $Reason = ""
    $Exclusions = $($VM.Tags)."VMSchedule-Exclusions"
    ...
    if (!$Exclude) {
        ...
        if ($TodayIsWeekday) {
            ...
            $ScheduledTime = $($VM.Tags)."VMSchedule-Weekdays"
        
        } elseif ($TodayIsWeekend) {

            $ScheduledTime = $($VM.Tags)."VMSchedule-Weekends"
        ...
        if ($ScheduledTime) {
            ...
            If (($TimeZoneAdjusted -gt $ScheduledStartTime) -and ($TimeZoneAdjusted -lt $ScheduledStopTime)) {
                ...
                #Current time is outside of the VMSchedule interval
            ...
            If ($VMAction -notlike "$VMActionExcluded") { #Make sure that action was not excluded
                ...
                Write-Output "VM $($VM.Name) is Excluded from changes during this run because VMSchedule-Exclusions Tag contains action $VMAction."
        ...
        Write-Output "VM $($VM.Name) is Excluded from changes during this run because VMSchedule-Exclusions Tag contains exclusion $Reason."
    }
